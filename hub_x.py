...
# --- INTERFAZ LOBBY ---
    ...
    if st.session_state.modulo_activo == "Lobby":
        ...
        cols = st.columns(4)
        servs = [("🚚 Logística", "1_Logistica.py"), ("🚜 Máquinas", "2_Maquinas.py"), ("🍔 Restaurante", "3_restaurante.py"), ("🐍 CACD", "x_cacd.py")]
                ...
                if acceso:
                    ...
                    if st.button(f"SUSCRIBIR - ${costo:,}", key=f"buy_{i}"):
                        if u["saldo"] >= costo:
                            ...
                            db[st.session_state.user_id]["movimientos"].append({"tipo": f"Suscripción {nom}", "monto": -costo, "fecha": datetime.now().strftime("%Y-%m-%d %H:%M")})
                            guardar_usuarios(db); st.rerun()
    else:
        if st.sidebar.button("⬅️ VOLVER AL LOBBY"): st.session_state.modulo_activo = "Lobby"; st.rerun()
