        ...
        if st.button("REGISTRAR EN EL SISTEMA"):
            if r_cel and r_pw and r_res:
                ...
                if r_cel in db: st.warning("El celular ya está registrado.")
                    ...
                    guardar_usuarios(db)
                    st.success(f"Cuenta {n_cuenta} creada. ¡Bienvenido!")
...
# --- INTERFAZ POST-LOGIN ---
    ...
    st.sidebar.write(f"💳 Cuenta: **{u['cuenta_f5co']}**")
    st.sidebar.metric("Saldo F5CO", f"${u['saldo']:,}")
    ...
    if st.session_state.modulo_activo == "Lobby":
        ...
        for i, (nom, file) in enumerate(servicios):
            with cols[i]:
                ...
                # Verificar suscripción activa (3 meses = 90 días)
                tiene_acceso = False
        ...
        with tab1: # 1. Cargar saldo
            ...
            if st.button("APLICAR ABONO"):
                if tid in db:
                    ...
                    guardar_usuarios(db); st.success("Abono cargado correctamente.")
                else: st.error("Usuario no encontrado.")

        with tab2: # 2. Consultar movimientos
            sid = st.text_input("Consultar Celular")
            if sid in db:
                st.write(f"### Estado de Cuenta: {db[sid]['nombre_completo']}")
                st.metric("Saldo Actual", f"${db[sid]['saldo']:,}")
        ...
        with tab3: # 3. Consultar/Modificar datos
            eid = st.text_input("Celular para Modificación")
            if eid in db:
                with st.form("edit_user"):
                    unom = st.text_input("Nombre Completo", value=db[eid]['nombre_completo'])
                    uusr = st.text_input("Username", value=db[eid]['username'])
                    uind = st.text_input("Indicio Pregunta", value=db[eid]['indicio'])
