...
# --- ESTÉTICA DARK TOTAL CON LETRAS BLANCAS ---
st.markdown("""
    <style>
    /* Fondo de la aplicación */
    .stApp { background-color: #000; color: #FFFFFF; }
    
    /* Forzar blanco en todas las etiquetas de formularios y textos */
    label, p, span, div, .stMarkdown, .stSubheader, .stTitle, .stHeader {
        color: #FFFFFF !important;
    }
    
    /* Estilo para las cajas de microservicios */
    .card { 
        background: #111; 
    ...
    /* Títulos en color cian para jerarquía */
    h1, h2, h3 { color: #00e6e6 !important; }
    
    /* Inputs con fondo oscuro y borde gris para visibilidad */
    input {
...
# --- MÓDULO: X USUARIOS ---
    ...
    with t_edit:
        ...
        if target in db_u:
            with st.form("edit_u"):
                u_dat = db_u[target]
                n_nom = st.text_input("Nombre Completo", value=u_dat.get('nombre_completo'))
